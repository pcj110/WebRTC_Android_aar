...
import static com.example.guan.webrtc_android_aar.common.GWebRTC_Helpers.createInstanceId;

/**
 * Created by guan on 3/24/17.
 */

public class GWebRTC_ClientManager {

    static String TAG = GWebRTC_AppRTC_Common.TAG_COMM + "ClientManager";
    ...
    private HashMap<String, GWebRTC_InstanceManager> map_instaces = new HashMap<>();
    ...
    public GWebRTC_ClientManager(Context mContext, int maxInstance, Handler handler) {
    ...
    public HashMap<String, GWebRTC_InstanceManager> getMap_instaces() {
        return map_instaces;
    }
    ...
    public void setSigParms(GWebRTC_AppRTC_Common.SignalingParameters sigParms) {
    ...
    public void createInstances() {
        ...
        for (int i = 0; i < maxInstance; i++) {
            ...
            GWebRTC_InstanceManager instanceManager = new GWebRTC_InstanceManager(mContext, sigParms,
                    instanceId, this,
                    handler);
    ...
    public GWebRTC_InstanceManager getInstance(String instanceId) {
        if (map_instaces.containsKey(instanceId)) {
    ...
    public void recreatePeerConnection(GWebRTC_InstanceManager instanceManager) {
    ...
    public void leaveRoomServer() {
        String leaveUrl = GWebRTC_AppRTC_Common.selected_WebRTC_URL + "/" + ROOM_LEAVE + "/" +
                GWebRTC_AppRTC_Common.selected_roomId + "/" + sigParms.clientId;

        GWebRTC_AsyncHttpURLConnection httpURLConnection = new GWebRTC_AsyncHttpURLConnection("POST",
                leaveUrl, "", new GWebRTC_AsyncHttpURLConnection.AsyncHttpEvents() {
            @Override
...
//        }
    ...
    public void requestConnectionAsInitiator(String localInstanceId) {
        ...
        GWebRTC_InstanceManager instanceManager = map_instaces.get(localInstanceId);
