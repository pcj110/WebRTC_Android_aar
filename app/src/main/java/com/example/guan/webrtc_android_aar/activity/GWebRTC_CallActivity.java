...
import com.example.guan.webrtc_android_aar.common.GWebRTC_ClientManager;
import com.example.guan.webrtc_android_aar.common.GWebRTC_InstanceManager;
import com.example.guan.webrtc_android_aar.common.GWebRTC_JsonHelper;
import com.example.guan.webrtc_android_aar.common.GWebRTC_VideoAudioHelper;
import com.example.guan.webrtc_android_aar.common.GWebRTC_WebSocketClient;
...
import com.example.guan.webrtc_android_aar.view.GWebRTC_AudioControlDialog;
...
import static com.example.guan.webrtc_android_aar.common.GWebRTC_AppRTC_Common.ROOM_JOIN;
import static com.example.guan.webrtc_android_aar.common.GWebRTC_AppRTC_Common.ROOM_LEAVE;
import static com.example.guan.webrtc_android_aar.common.GWebRTC_AppRTC_Common.VIDEO_CODEC_VP8;
import static com.example.guan.webrtc_android_aar.common.GWebRTC_AppRTC_Common.selected_roomId;
import static com.example.guan.webrtc_android_aar.common.GWebRTC_Helpers.preferCodec;
import static com.example.guan.webrtc_android_aar.common.GWebRTC_JsonHelper.toJavaCandidate;

/**
 * Created by guan on 4/5/17.
 */

public class GWebRTC_CallActivity extends AppCompatActivity {

    //类相关
    private static Context mContext;
    private static String TAG = GWebRTC_AppRTC_Common.TAG_COMM + "CallActivity";
    ...
    GWebRTC_CallActivity.WSMessageEvent wsMessageEvent;
    GWebRTC_WebSocketClient wsClient;
    GWebRTC_ClientManager clientManager;

    //辅助相关
    GWebRTC_VideoAudioHelper vaHelper;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_call_gwebrtc);
    ...
    public void onWindowFocusChanged(boolean hasFocus) {
        ...
        GWebRTC_AppConstant.RENDERERHEIGHT = renderer_layout.getHeight();
        GWebRTC_AppConstant.RENDERERWIDTH = renderer_layout.getWidth();

    }

    private void initData() {
        mContext = GWebRTC_CallActivity.this;

        clientManager = new GWebRTC_ClientManager(this, MaxInstance, handler);
    }
    //==============================================================
    ...
    private void initUI() {
        ...
        roomID_tv.setText(GWebRTC_AppRTC_Common.selected_roomId);
        ...
        cancel_btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                final GWebRTC_YesOrNoDialog yesOrNoDialog = new GWebRTC_YesOrNoDialog(GWebRTC_CallActivity.this);
                yesOrNoDialog.setMeesage("您确定要退出房间吗？");
                yesOrNoDialog.setCallback(new GWebRTC_YesOrNoDialog.YesOrNoDialogCallback() {
                    @Override
                        ...
                        if (button == GWebRTC_YesOrNoDialog.ClickedButton.POSITIVE) {
                            ...
                            handler.post(new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    for (final Map.Entry<String, GWebRTC_InstanceManager> entry : clientManager.getMap_instaces().entrySet()) {
                            ...
                            GWebRTC_CallActivity.this.setResult(RESULT_OK);
                            GWebRTC_CallActivity.this.finish();
...
//                }
    ...
    {
        final GWebRTC_InstanceManager instanceManager = clientManager.getInstance(localInstanceId);

        final GWebRTC_AudioControlDialog audioControlDialog=new GWebRTC_AudioControlDialog(mContext);
        audioControlDialog.setSwitchState(instanceManager.isLocalAudioState(),instanceManager.isRemoteAudioState());
        audioControlDialog.setCallback(new GWebRTC_AudioControlDialog.AudioControlDialogCallback() {
            @Override
    ...
    private void joinRoom() {
        String roomJoin_Url = GWebRTC_AppRTC_Common.selected_WebRTC_URL + "/" + ROOM_JOIN + "/" +
                GWebRTC_AppRTC_Common.selected_roomId;
        String roomMessage = "";

        Log.d(TAG, "Connecting to room: " + roomJoin_Url + "\troomMessage:" + roomMessage);

        GWebRTC_AsyncHttpURLConnection httpConnection =
                new GWebRTC_AsyncHttpURLConnection("POST", roomJoin_Url, null, new GWebRTC_AsyncHttpURLConnection.AsyncHttpEvents() {
                    @Override
                    ...
                    public void onHttpComplete(String response) {
                        //开始解析返回的参数
                        final GWebRTC_AppRTC_Common.SignalingParameters sigParms =
                                GWebRTC_JsonHelper.roomHttpResponseParse(response);
                        ...
                        if (sigParms.result.equals("ROOM_FULL")) {
                            ...
                            GWebRTC_CallActivity.this.runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                                    initClient(sigParms);
                                    showLocalVideo();
     ...
     * @param sigParams
     */
    private void initClient(GWebRTC_AppRTC_Common.SignalingParameters sigParams) {
    ...
    private boolean checkIfRoomIsCreated(boolean initiator, String clientId) {
        String roomLeave_Url = GWebRTC_AppRTC_Common.selected_WebRTC_URL + "/" + ROOM_LEAVE + "/" +
                GWebRTC_AppRTC_Common.selected_roomId + "/" + clientId;

        if (initiator) {
            if (GWebRTC_AppRTC_Common.selected_role == GWebRTC_AppRTC_Common.RoomRole.SLAVE) {
                showToast("没有找到该房间:" + selected_roomId + "\t请先创建房间，再加入！");
                leaveRoomAndFinishActivity(GWebRTC_AppRTC_Common.MessageType.LEAVE,
                        roomLeave_Url,
                        null);
                return false;
            } else {
            ...
            if (GWebRTC_AppRTC_Common.selected_role == GWebRTC_AppRTC_Common.RoomRole.MASTER) {
    ...
    private void leaveRoomAndFinishActivity(final GWebRTC_AppRTC_Common.MessageType messageType,
                                            final String url,
    ...
    private void showLocalVideo() {
        ...
        try {
            ...
            vaHelper = new GWebRTC_VideoAudioHelper(mContext, factory, localRender);
    ...
    private void startCall() {
        ...
        handler.post(new Runnable() {
            ...
            public void run() {
                ...
                GWebRTC_AppRTC_Common.SignalingParameters sigParms = clientManager.getSigParms();

                wsMessageEvent = new GWebRTC_CallActivity.WSMessageEvent();
                wsClient = new GWebRTC_WebSocketClient(handler,
                        wsMessageEvent,
                        sigParms.roomId);
                ...
                for (HashMap.Entry entry : clientManager.getMap_instaces().entrySet()) {
                    ...
                    GWebRTC_InstanceManager instanceManager = (GWebRTC_InstanceManager) entry.getValue();
                    ...
                    parametersReady((GWebRTC_InstanceManager) entry.getValue(), roomSize);
    ...
    private boolean allocateResources(GWebRTC_InstanceManager instanceManager) {
        ...
        //创建并指定PCObserver
        GWebRTC_CallActivity.PCObserver pcObserver = new GWebRTC_CallActivity.PCObserver(instanceManager);
        ...
        //创建并指定SDPObserver
        GWebRTC_CallActivity.SDPObserver sdpObserver = new GWebRTC_CallActivity.SDPObserver(instanceManager);
...
//        }
    ...
    private void parametersReady(final GWebRTC_InstanceManager instanceManager, final int count) {
    ...
    public class PCObserver implements PeerConnection.Observer {
        ...
        private GWebRTC_InstanceManager instanceManager;
        //private IceCandidate localcandidate;

        private MediaStream remoteMediaStream;

        public PCObserver(GWebRTC_InstanceManager instanceManager) {
    ...
    public class SDPObserver implements SdpObserver {
        ...
        public SDPObserver(GWebRTC_InstanceManager instanceManager) {
            this.instanceManager = instanceManager;
        }
...
//            }
     ...
     *
     */
    public class WSMessageEvent implements GWebRTC_WebSocketClient.IWebSocketMessageEvent {

        private String TAG = GWebRTC_AppRTC_Common.TAG_COMM + "WSMessageEvent";
        ...
        public void onWebSocketMessage(String msg) {
            ...
            try {
                ...
                if (msgText.length() > 0) {
                    ...
                    final GWebRTC_InstanceManager instanceManager = clientManager.getInstance(receiverId);
                    ...
                    if (type.equals("candidate")) {
                        ...
                        //instanceManager.ic_action_hung_up();
        ...
        GWebRTC_CallActivity.this.runOnUiThread(new Runnable() {
            ...
            public void run() {
                if (isMute) {
                    ...
                    layoutParams.width = GWebRTC_AppConstant.RENDERERWIDTH / 2;
                    layoutParams.height = (GWebRTC_AppConstant.RENDERERHEIGHT) / 2;
